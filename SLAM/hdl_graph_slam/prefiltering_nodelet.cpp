...
private:
  void initialize_params() {
    ...
    N_SCAN = private_nh.param<int>("N_SCAN", 128);
    downsampleRate = private_nh.param<int>("downsampleRate", 1);
  ...
  pcl::PointCloud<PointT>::ConstPtr distance_filter(const pcl::PointCloud<PointT>::ConstPtr& cloud) const {
    ...
    std::copy_if(cloud->begin(), cloud->end(), std::back_inserter(filtered->points), [&](const PointT& p) {
      ...
      double deg = (atan(p.z/r) * 180 / M_PI) + 25.0;
      int rowIdn = (int)floor(deg*128.0/40.0);
      ...
      return r > distance_near_thresh && r < distance_far_thresh;
    });
