...
class NuScenesEval:
    def __init__(self, pred_label_path, gt_label_path, label_format, save_loc,
                 ...
                 min_range=0.0,
                 max_range=100.0):
        assert (min_range>=0) and (max_range>0)
    ...
    def evaluate(self):
        pred_file_list = glob.glob(os.path.join(self.pred_label_path, "*"))
        pred_file_list.sort()
        gt_file_list = glob.glob(os.path.join(self.gt_label_path + "*"))
        gt_file_list.sort()
        ...
        for pred_fn in pred_file_list:
            if (self.gt_label_path + os.path.basename(pred_fn)) not in gt_file_list:
        ...
        file_parsing = LabelParser(self.label_format)
        for i, pred_fn in enumerate(pred_file_list):
            # print("\r", i+1, "/", num_examples, end="")
            gt_fn = self.gt_label_path + os.path.basename(pred_fn)
            predictions = file_parsing.parse_label(pred_fn, prediction=True)
            ground_truth = file_parsing.parse_label(gt_fn, prediction=False)
            # Filter range
            if self.max_range > 0:
                predictions, ground_truth = self.filter_by_box(predictions, ground_truth, min_range=self.min_range, max_range=self.max_range)
                # predictions, ground_truth = self.filter_by_range(predictions, ground_truth, min_range=self.min_range, max_range=self.max_range)
        ...
        for single_class in self.classes:
            ...
            mAP_dict[single_class] = {
                "Number_GT": class_dict['total_N_pos'],
                "Total_Detection": class_dict['result'].shape[0],
                "TP": np.sum(class_dict['result'][:, 0] == 1),
                "FP": np.sum(class_dict['result'][:, 0] == 0),
                "mAP": mean_ap,
                "F1_Score": f1,
                "Avg_Translation_Error_2D": ate2d,
                "Avg_Translation_Error_3D": ate3d,
                "Avg_Scale_Error": ase,
                "Avg_Rotation_Error": aoe,
            }
    ...
    def filter_by_box(self, pred_label, gt_label, min_range=0, max_range=100):
        pred_sel = (np.abs(pred_label[:, 1].astype(float))>=min_range) & (np.abs(pred_label[:, 1].astype(float))<max_range) & (np.abs(pred_label[:, 2].astype(float))>=min_range) & (np.abs(pred_label[:, 2].astype(float))<max_range)
        gt_sel = (np.abs(gt_label[:, 1].astype(float))>=min_range) & (np.abs(gt_label[:, 1].astype(float))<max_range) & (np.abs(gt_label[:, 2].astype(float))>=min_range) & (np.abs(gt_label[:, 2].astype(float))<max_range)
        return pred_label[pred_sel, :], gt_label[gt_sel, :]

    def filter_by_range(self, pred_label, gt_label, min_range=0, max_range=100):
        pred_dist = np.linalg.norm(pred_label[:, 1:3].astype(float), axis=1)
        pred_dist = (pred_dist>=min_range) & (pred_dist<=max_range)

        gt_dist = np.linalg.norm(gt_label[:, 1:3].astype(np.float32), axis=1)
        gt_dist = (gt_dist>=min_range) & (gt_dist<=max_range)
